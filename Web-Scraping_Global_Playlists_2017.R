...
playlist_specs <- list(
  TTH = list(short = "TTH", long = "Todays_Top_Hits", id = "879549", start = "2017-05-03"),
  RC  = list(short = "RC",  long = "Rap_Caviar",      id = "127201", start = "2017-03-03"),
  VL  = list(short = "VL",  long = "Viva_Latino",     id = "879552", start = "2017-05-03"),
  BR  = list(short = "BR",  long = "Baila_Reggaeton", id = "511794", start = "2017-04-16")
)

# Per-year constants (only these three lines change for the 2024 script)
ISRC_YEAR_CUTOFF <- 17
END_YEAR_STR     <- "2017-12-31"
FILTER_YEAR      <- "2017"
...
for (key in names(playlist_specs)) {
  ...
  result <- process_editorial_playlist(spec, session, Charts,
                                       streams_per_country, months)
  assign(paste0(spec$long, "_2017"), result$playlist)
  assign(paste0(spec$short, "_Charts_2017"), result$charts)
}
