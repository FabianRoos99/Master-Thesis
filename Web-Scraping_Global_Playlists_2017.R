...
# Relevant timeframe (2016 & 2017) -----------------------------------

months <- seq(as.Date("2016-01-01"), as.Date("2017-12-31"), by = "1 month")
...
# Charts_filled <- Charts %>%
#   group_by(song_id, country) %>%
#   arrange(date) %>%
#   mutate(date = as_date(date)) %>%
#   mutate(
#     position = as.numeric(position),
#     gap = as.integer(difftime(date, lag(date), units = "days")),
#     fill_needed = ifelse(gap == 2, TRUE, FALSE)
#   ) %>%
#   ungroup() %>%
#   filter(!is.na(date)) %>%
#   group_by(song_id, country) %>%
#   tidyr::complete(date = full_seq(date, 1)) %>%
#   fill(song_id, title, artist, country, .direction = "downup") %>%
#   mutate(
#     fill_needed = ifelse(is.na(gap) & !is.na(lag(gap)) & lag(gap) <= 2, TRUE, fill_needed),
#     position = ifelse(!is.na(position) | fill_needed, position, NA),
#     streams  = ifelse(!is.na(streams) | fill_needed, streams, NA)
#   ) %>%
#   mutate(
#     position = ifelse(fill_needed, zoo::na.approx(position, na.rm = FALSE), position),
#     streams = ifelse(fill_needed, zoo::na.approx(streams, na.rm = FALSE), streams)
#   ) %>%
#   filter(!is.na(position) | !is.na(streams)) %>%
#   mutate(gap_next = lead(gap)) %>%
#   filter(is.na(gap_next) | gap_next <= 2) %>%
#   select(-gap, -fill_needed, -gap_next) %>%
#   ungroup()

# total streams per country for 2017
...
Todays_Top_Hits$release_year <- ifelse(as.numeric(substr(Todays_Top_Hits$isrc, 6, 7)) <= 17,
...
Rap_Caviar$release_year <- ifelse(as.numeric(substr(Rap_Caviar$isrc, 6, 7)) <= 17,
...
# normalized streams and filtering for 2017-data
RC_Charts <- RC_Charts %>%
  filter(substr(date, 1, 4) == "2017") %>%
  left_join(streams_per_country, by = "country") %>%
  mutate(normalized_streams = streams / total_streams * 1000000)
...
Viva_Latino$release_year <- ifelse(as.numeric(substr(Viva_Latino$isrc, 6, 7)) <= 17,
...
Baila_Reggaeton$release_year <- ifelse(as.numeric(substr(Baila_Reggaeton$isrc, 6, 7)) <= 17,
